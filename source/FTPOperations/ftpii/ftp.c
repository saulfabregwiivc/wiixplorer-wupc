...
static s32 ftp_PASV(client_t *client, char *rest UNUSED) {
	...
	struct in_addr addr;
	addr.s_addr = ip;
	gxprintf("Listening for data connections at %s:%u...\n", inet_ntoa(addr), port);
...
static s32 ftp_PORT(client_t *client, char *portspec) {
	...
	gxprintf("Set client address to %s:%u\n", addr_str, port);
...
static s32 prepare_data_connection_active(client_t *client, data_connection_callback callback UNUSED, void *arg UNUSED) {
	...
	gxprintf("Attempting to connect to client at %s:%u\n", inet_ntoa(client->address.sin_addr), ntohs(client->address.sin_port));
	return 0;
}
...
static bool process_accept_events(s32 server) {
	...
	while ((peer = net_accept(server, (struct sockaddr *)&client_address, &addrlen)) != -EAGAIN) {
		if (peer < 0) {
			gxprintf("Error accepting connection: [%i] %s\n", -peer, strerror(-peer));
			return false;
		}

		gxprintf("Accepted connection from %s!\n", inet_ntoa(client_address.sin_addr));

		if (num_clients == MAX_CLIENTS) {
			gxprintf("Maximum of %u clients reached, not accepting client.\n", MAX_CLIENTS);
			net_close(peer);
			return true;
		}
		...
		if (!client) {
			gxprintf("Could not allocate memory for client state, not accepting client.\n");
			net_close(peer);
			return true;
		}
		client->socket = peer;
		...
		*client->pending_rename = '\0';
		...
		if (write_reply(client, 220, "ftpii") < 0) {
			gxprintf("Error writing greeting.\n");
	...
	if (!client->data_connection_connected) {
		if (client->passive_socket >= 0) {
			...
			if ((result = net_connect(client->data_socket, (struct sockaddr *)&client->address, sizeof(client->address))) < 0) {
				...
				if (result != -EAGAIN && result != -EISCONN) gxprintf("Unable to connect to client: [%i] %s\n", -result, strerror(-result));
			}
		...
		if (client->data_connection_connected) {
			result = 1;
			gxprintf("Connected to client!  Transferring data...\n");
		} else if (gettime() > client->data_connection_timer) {
			result = -1;
			gxprintf("Timed out waiting for data connection.\n");
		}
	} else {
...
static void process_control_events(client_t *client) {
	...
	while (client->offset < (FTP_BUFFER_SIZE - 1)) {
		...
		if ((bytes_read = net_read(client->socket, offset_buf, FTP_BUFFER_SIZE - 1 - client->offset)) < 0) {
			if (bytes_read != -EAGAIN) {
				gxprintf("Read error %i occurred, closing client.\n", bytes_read);
				goto recv_loop_end;
			}
			return;
		...
		if (strchr(offset_buf, '\0') != (client->buf + client->offset)) {
			gxprintf("Received a null byte from client, closing connection ;-)\n"); // i have decided this isn't allowed =P
			goto recv_loop_end;
		}
		...
		for (next = client->buf; (end = strstr(next, CRLF)) && !client->data_callback; next = end + CRLF_LENGTH) {
			*end = '\0';
			if (strchr(next, '\n')) {
				gxprintf("Received a line-feed from client without preceding carriage return, closing connection ;-)\n"); // i have decided this isn't allowed =P
				goto recv_loop_end;
			}
	...
	gxprintf("Received line longer than %u bytes, closing client.\n", FTP_BUFFER_SIZE - 1);
